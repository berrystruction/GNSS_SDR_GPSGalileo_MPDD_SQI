function [covarianceMTX, covariancePosVelocityMTX]  = covNoiseMakerStatic(trackres,currMeasNr,lenChList)
%COVNOISEMAKER
...
if nargout==2
    ...
    % b=40^6; % [m^2/s^2*Hz]
    sigma2_i=a+b*10.^(-CN0sorted/10);
    covariancePosVelocityMTX=zeros(2*Nmeasurement+6);
    ...
    for index=2*Nmeasurement+1:2*Nmeasurement+6
        covariancePosVelocityMTX(index,index)=1e-4;
    end
