...
if (fid > 0)
    ...
    % Read 10ms of signal
    acqNsamp = [1, 10*samplesPerCode*settings.NSample];
    
    [data, count] = fread(fid, acqNsamp, settings.dataType);
    ...
    if strcmp(settings.signalType,'IQ')==1
        plot(1000 * timeScale(1:round(samplesPerCode/50)), ...
            abs(data(1:round(samplesPerCode/50))));
    else
        plot(1000 * timeScale(1:round(samplesPerCode/50)), ...
    ...
    subplot(2,2,2);
    if strcmp(settings.signalType,'IQ')==1
        plot(fftshift(abs(fft(data(1:2:end-1)+1i*data(2:2:end))))), grid on
    else
