...
function [doppler, code, st, C,peakMetric] = signal_acquisition_FFT_RAPID(data,settings,prn)%(file,sampling_freq,IF,prn,T_int,seek_sec)
...
%wbh = waitbar(0,'"Rapid" signal acquisition. Please wait...');
j=sqrt(-1);

% T_int = 4e-3; %1e-3; %20e-3;     % [s] coherent integration time
% T_int = CodeLen/Rc;    % [s] coherent integration time = 1 primary code period (= 4 ms)
%global T_int;
Non_Coh_Sums = settings.Non_Coh_Sums; %3 %5; %11; %50; %20; %100; % Non coherent summations (>= 1)
...
acq_metric = settings.acqThreshold;%1.4; % Acquisition metric
...
for ind_FD= 1:length(FD_vect)
    ...
    for M=0:(Non_Coh_Sums-1)
        ...
        SigLOCFFT=conj(fft(SigLOC,N));
        if strcmp(settings.signalType,'IQ')==1%(IF==0) % IQ sampling
            %%%% for complex baseband signal
            SigIN_I = data(2*N*M+1:2:2*N*M+2*N-1);
            SigIN_Q = data(2*N*M+2:2:2*N*M+2*N);
            
            I = SigIN_I.*carrQ - SigIN_Q.*carrI;
            Q = SigIN_I.*carrI + SigIN_Q.*carrQ;
        else % IF sampling
            % Dati ricevuti
