function plotNavigation2(navSolutions, settings,xtime,navSolutionsRef)
...
if (~isempty(navSolutions))
    ...
    if isnan(settings.truePosition.E) || isnan(settings.truePosition.N) ...
        ...
        refCoord.E = mean(navSolutionsRef.E(~isnan(navSolutionsRef.E)));
        refCoord.N = mean(navSolutionsRef.N(~isnan(navSolutionsRef.N)));
        refCoord.U = mean(navSolutionsRef.U(~isnan(navSolutionsRef.U)));
...
%             mean(navSolutions.latitude(~isnan(navSolutions.latitude)))), -5);
        meanLongitude = degrees2dms(...
            mean(navSolutionsRef.longitude(~isnan(navSolutionsRef.longitude))));
        meanLatitude  = degrees2dms(...
            mean(navSolutionsRef.latitude(~isnan(navSolutionsRef.latitude))));
        ...
        refPointLgText = ['Mean Position\newline  Lat: ', ...
                         ...
                         '\newline Hgt: ', ...
                            num2str(mean(navSolutionsRef.height(~isnan(navSolutionsRef.height))), '%+6.1f')];
    else
