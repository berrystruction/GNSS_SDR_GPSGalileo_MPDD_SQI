function [new_u, new_d]=Multicorrelator(localcode,tempdatacos,tempdatasin,codephasestep,remcodephase,blksize,minlag,stepp,maxlag)
ca=localcode;
...
for lag=minlag:stepp:maxlag
    ...
    pp_e(vecindex)= sign(p_i)*sign(e_i(vecindex))*sqrt(e_i(vecindex).^2+e_q(vecindex).^2);
    pp_l(vecindex)= sign(p_i)*sign(l_i(vecindex))*sqrt(l_i(vecindex).^2+l_q(vecindex).^2);
