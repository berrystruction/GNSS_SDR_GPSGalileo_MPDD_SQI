function [channel,FLL_results] = preRun(file,acqResults,settings)
...
FLL_results.StatusFLL= zeros(1,32);
FLL_results.cnt_skp= zeros(1,32);
FLL_results.doppler_estFLL= zeros(1,32);
FLL_results.phaseFLL= zeros(1,32);
FLL_results.seeksec_corr= zeros(1,32);
...
for ii = 1:min([settings.numberOfChannels, sum(acqResults.peakMetric>=settings.acqThreshold)])
    ...
    %% FLL ================================================================
    FLLres=trackcarrFLL(file,acqResults,settings,PRNindexes(ii));
    % copying results...
    FLL_results.StatusFLL(PRNindexes(ii))=FLLres.StatusFLL;
    FLL_results.cnt_skp(PRNindexes(ii))=FLLres.cnt_skp;
    FLL_results.doppler_estFLL(PRNindexes(ii))=FLLres.doppler_estFLL;
    FLL_results.phaseFLL(PRNindexes(ii))=FLLres.phaseFLL;
    FLL_results.seeksec_corr(PRNindexes(ii))=FLLres.phaseFLL/settings.samplingFreq-settings.seek_sec;
    
    
    channel(ii).acquiredFreq = FLL_results.doppler_estFLL(PRNindexes(ii));
    channel(ii).codePhase    = FLL_results.phaseFLL(PRNindexes(ii));
