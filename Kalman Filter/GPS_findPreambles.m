function [firstSubFrame] = GPS_findPreambles(trackResults,Tc,settings)
...
%searchStartOffset = 40000;
searchStartOffset = settings.searchStartOffset;

%--- Initialize the firstSubFrame array -----------------------------------
firstSubFrame = zeros(1, length(trackResults));
...
%% Correlate tracking output with preamble ================================
    ...
    if (Tc == 0.02)
        % "Upsample" - make 20 values per one bit.
        bits = kron(bits, ones(1, 20));
    end
    ...
    for i = 1:size(index) % For each occurrence
        ...
        if (~isempty(find(index2 == 6000)))
            ...
            if (Tc == 0.001)
                ...
                bits = sum(bits);
            elseif (Tc == 0.02)
                bits = trackResults(channelNr).I_P((index(i)+19)/20-2 : ...
                    (index(i)+19)/20 + 60 -1);
            end
...
% % %                        i = 3;
% % %                    end
                    firstSubFrame(channelNr) = (index(i)+19)/20;
                else
                    disp('Error in setting the L1 integration time');
                end
                break;    
            end % if parity is OK ...
