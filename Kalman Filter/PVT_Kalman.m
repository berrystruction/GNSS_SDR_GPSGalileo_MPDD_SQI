function [navSolutions,RAIMresults,unreliableSol,TGlobalTest,TlocalTest,GlobalThres,remainingSV,SVexcluded]=PVT_Kalman(trackResults,settings,kalmansettings,LSorWLS,SQI,SQIthreshold)
%% FILE to LOAD
...
%% ATTENTION!!! This overwrites the content of settings in TrackResults!!
...
%     end
    ...
    if (settings.CS.enable_smoothL1)
        ...
        for i11 = activeChnList_GPS_L1
            ...
            pseudo_smoothedL1(i11,currMeasNr) = 1/settings.CS.smoothL1_weights * pseudo_codeL1(i11,currMeasNr) + ...
                (settings.CS.smoothL1_weights-1) / settings.CS.smoothL1_weights * pseudo_PhaseL1(i11,currMeasNr);
        end
    end
    ...
    if currMeasNr < startKalman
        ...
        if(settings.CS.enable_smoothL1 == 1)
            if(currMeasNr == 1)
            ...
            all_is_ok=-1;
            RecomputePVT=0;
            pseudoCumul=0; % da completare
            [cov_noise]  = covNoiseMaker(channels_GPS_L1(activeChnList_GPS_L1),currMeasNr,length(activeChnList_GPS_L1),pseudoranges1_GPS_L1(activeChnList_GPS_L1),...,
                pseudoCumul,SQI([channels_GPS_L1(activeChnList_GPS_L1).PRN],currMeasNr),SQIthreshold,settings.SQIpenalty);
            ...
            while(all_is_ok==-1 && length(activeChnList_GPS_L1)>=4)
                ...
                % ... RAIM algorithms
                if settings.RAIM.type==1
                    % Solution Separation Method
                    [all_is_ok, measurIndtoexclude, TGlobalTest(currMeasNr),TlocalTest(currMeasNr),GlobalThres,PL]...
                        =FaultDetectionSS(pos,RAIMresult,settings,cov_noise,activeChnList_GPS_L1,satpos, pseudoranges1_GPS_L1,channels_GPS_L1, Doppler_GPS_L1, settings);

                    
                else
                    
                    if length(activeChnList_GPS_L1)>=6 && settings.RAIM.enableRAIM==true && RecomputePVT<settings.RAIM.RecomputePVT
                        %                     RAIMresults(currMeasNr) = RAIMresult;
                        [all_is_ok, measurIndtoexclude, TGlobalTest(currMeasNr),TlocalTest(currMeasNr),GlobalThres,PL]=...
                            FaultDetection(pos,RAIMresults(currMeasNr),settings,cov_noise,SQI([channels_GPS_L1(activeChnList_GPS_L1).PRN],currMeasNr));
...
%             newdiag= 1e2*([10*Sp/3 10*Sp/3 10*Sp/3 (Sf+Sg/2) 1*Sp 1*Sp 1*Sp Sg]);
%             for auss=1:8
%                 KGPS.QStateNoiseCovariance(auss,auss) =newdiag(auss);
%             end
%             %startKalman=currMeasNr+7;
%         end
        ...
        while(all_is_ok==-1 && length(activeChnList_GPS_L1)>=4)
            ...
            pseudo=0; % da finire
            KGPS = initSigmaKalmanGPS(KGPS,channels_GPS_L1(activeChnList_GPS_L1),currMeasNr,kalmansettings.staticposition,pseudo,SQI([channels_GPS_L1(activeChnList_GPS_L1).PRN],currMeasNr),SQIthreshold,settings.SQIpenalty);
...
%                 end
            ...
            if length(activeChnList_GPS_L1)>=6 && settings.RAIM.enableRAIM==true && RecomputePVT<settings.RAIM.RecomputePVT
                ...
                [all_is_ok, measurIndtoexclude, TGlobalTest(currMeasNr),TlocalTest(currMeasNr),GlobalThres,PL]=FaultDetection(pos,RAIMresults(currMeasNr),settings,...
                    KGPS.RObservationNoiseCovariance(1:length(activeChnList_GPS_L1),1:length(activeChnList_GPS_L1)),SQI([channels_GPS_L1(activeChnList_GPS_L1).PRN],currMeasNr),1);
                %all_is_ok=1; % To avoid the exclusion
                % Exclusion
                ExclusionSatellites
            else
